...
declare
	...
	new_id_street int;
BEGIN
	if not exists(select * from city where NameCity = _NameCity) then
		begin
			select COALESCE(max(IDCity) + 1,0) into new_id_city from city;
			insert into city (IDCity,NameCity) values (new_id_city,_NameCity);
		end;
	else
		select IDCity into new_id_city from city where NameCity = _NameCity;
	end if;
...
select del_sight_clear_typesight(8);
select del_sight_clear_typesight(11);
