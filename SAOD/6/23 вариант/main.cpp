/*
  ...
  с вариантом задания;
    ...
    Для каждого узла дерева A, вывести списко потомков в порядке обхода
*/
...
int menu() {
  while (true) {
    ...
    cout << "6) Обход дерева (симетрично - inOrder)" << endl;
		cout << "7) Вывод список потомков для узла в порядке обхода" << endl;
    cout << "0) Выход" << endl;
    cout << " > ";
    int id = read_double();
    if (0 <= id <= 7) {
...
int main() {
  ...
  while (true) {
    ...
    switch (menu_i) {
      ...
      case (7): {
				cout << "Элемент у которого буду выведены потомки: ";
        double a = read_double();
				Node* find_element = tree.find(a, false);
        if (find_element != NULL) {
					tree.inOrder(find_element);
        } else cout << "Такого элемента не существет." << endl;
        break;
			}
