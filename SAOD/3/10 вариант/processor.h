...
public:
  ...
  Processors(MYList *stack1, int size_queue1, MYList* _queue1, MYList* _queue2, MYList* _queue3, Generator* generator1);
  void tick();
  void loop();
};

// конструктор
Processors::Processors(MYList *stack1, int size_queue1, MYList* _queue1, MYList* _queue2, MYList* _queue3, Generator* generator1) {
  stack = stack1;
  ...
  int count = (size_queue < generator -> get_size())? size_queue: generator -> get_size();
  ...
  count = (size_queue < generator -> get_size())? size_queue: generator -> get_size();
  for (int i = 0; i < count; i++) {
    buf = generator -> get();
    queue2 -> append(
      buf.priority,
...
void Processors::tick() {
  ...
  if (queue3 -> get_size() < size_queue && generator -> get_size() > 0) {
